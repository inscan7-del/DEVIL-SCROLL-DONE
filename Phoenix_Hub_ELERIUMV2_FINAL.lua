...
local window = library:AddWindow("🔥سكربت خاص✅| تم التطوير من قبل فهد و يوسف", {
...
-- Added from PHOENIX HUB Inventory without modifying the original DEVIL HUB code.
...
local function buySelectedPet()
    ...
    if not remote then
        warn("PHOENIX HUB: cPetShopRemote not found.")
        return false
    end
    if not petObject then
        warn("PHOENIX HUB: Selected pet was not found in cPetShopFolder.")
        return false
    end
    ...
    if not ok then
        warn("PHOENIX HUB: Pet purchase request failed: " .. tostring(result))
        return false
    end
