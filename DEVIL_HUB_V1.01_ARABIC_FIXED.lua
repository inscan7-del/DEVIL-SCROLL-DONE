...
local function createEnemyالصحةBar(player)
    ...
    table.insert(enemyالصحةConnections[player], humanoid.HealthChanged:Connect(update))
    ...
    if enabled then
        task.spawn(function()
            while farmingTarget do
                ...
                if targetإعادة الميلادAmount and currentRebirths >= targetإعادة الميلادAmount then
            ...
            if player then
                ...
                if leaderstats then
                    ...
                    if leaderstats:FindFirstChild("Rebirths") then
                        Update2.Text = "إعادة الميلاد: " .. FormatDisplay(leaderstats.Rebirths.Value)
                    end
...
do
    ...
    MainParity:AddTextBox("تحديد هدف إعادة الميلاد", function(v)
        local n = tonumber(v)
